...
export function like_router(pool: Pool): Router {
  ...
  router.post('/like', (req: Request<{ UID: number, PID: number }>, res, next) => { post_like_handler_p(req, res, pool, next) });
  router.post('/unlike', (req: Request<{ UID: number, PID: number }>, res, next) => { post_unlike_handler_p(req, res, pool, next) });
